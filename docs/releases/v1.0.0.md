...
Eveable v1.0.0 is the first stable release of an alternative to Lovable built on Vercel Eve. It keeps the existing NestJS `jaxagentsdk` untouched and provides a standalone Eve implementation that developers can run, inspect, extend, and deploy independently.
...
- Eve-native local tools for generated file writes, finite quality commands, preview process startup, Vercel deployment, and Unsplash image search.
...
Eveable routes every user message through the root Eve agent. The root calls the intent subagent first, refuses unsafe requests before builder agents, sends normal chat to the conversation subagent, and sends build requests through the builder chain.
...
9. `start_preview`: start the preview process and verify it over HTTP.
10. `security_review`: check for secret exposure, unsafe browser env usage, and common web app risks.
11. `deploy_to_vercel`: deploy the generated app and verify the Vercel URL.
12. `conversation`: produce the final user-facing summary.
...
- Generated code is stored in the Eve sandbox under `/workspace/generated-app`; the public preview URL is created by `deploy_to_vercel`.
- The current Eve/provider combination rejects valid JSON text when optional subagent `outputSchema` is passed for this workflow. Eveable keeps shared Zod schemas for developer reference and asks subagents for plain JSON text in the `message` field.
